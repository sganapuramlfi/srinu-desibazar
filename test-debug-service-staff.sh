...
STAFF_SKILLS_RESPONSE=$(curl -s -b cookies.txt "http://localhost:5000/api/businesses/$BUSINESS_ID/staff-skills")
...
if [ -n "$STAFF_ID" ]; then
  ...
  VERIFY_SKILLS_RESPONSE=$(curl -s -b cookies.txt "http://localhost:5000/api/businesses/$BUSINESS_ID/staff/$STAFF_ID/skills")
