...
curl -s -b cookies.txt "http://localhost:5000/api/businesses/$BUSINESS_ID/services" | jq .
...
NEW_SERVICE_RESPONSE=$(curl -X POST -H "Content-Type: application/json" \
  ...
  -b cookies.txt \
  -s "http://localhost:5000/api/businesses/$BUSINESS_ID/services")
...
curl -s -b cookies.txt "http://localhost:5000/api/businesses/$BUSINESS_ID/staff" | jq .
...
NEW_STAFF_RESPONSE=$(curl -X POST -H "Content-Type: application/json" \
  ...
  -b cookies.txt \
  -s "http://localhost:5000/api/businesses/$BUSINESS_ID/staff")
...
for route in "" "/services" "/staff" "/bookings" "/analytics"; do
  ...
  response=$(curl -s -o /dev/null -w "%{http_code}" -b cookies.txt "http://localhost:5000/dashboard/$BUSINESS_ID$route")
...
curl -s -b cookies.txt "http://localhost:5000/dashboard/$BUSINESS_ID/services" | grep -q "Service Management" && echo "✅ Service management component loaded" || echo "❌ Service management component failed to load"

# Test staff list component
echo "Testing staff list component"
curl -s -b cookies.txt "http://localhost:5000/dashboard/$BUSINESS_ID/staff" | grep -q "Staff Management" && echo "✅ Staff management component loaded" || echo "❌ Staff management component failed to load"

# Test booking management component
echo "Testing booking management component"
curl -s -b cookies.txt "http://localhost:5000/dashboard/$BUSINESS_ID/bookings" | grep -q "Booking Management" && echo "✅ Booking management component loaded" || echo "❌ Booking management component failed to load"
